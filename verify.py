...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS settings
                 (guild_id TEXT PRIMARY KEY, role_id TEXT, log_channel_id TEXT, redirect_url TEXT)''')
    conn.commit()
    conn.close()
...
@bot.tree.command(name="verify-web", description="認証パネルを設置します")
@app_commands.describe(
    role="認証後に付与するロール",
    log_channel="認証ログを送信するチャンネル",
    panel_channel="パネルを設置するチャンネル",
    title="パネルのタイトル",
    description="パネルの説明文",
    button_text="ボタンの文字",
    redirect_url="ボタンを押した時に飛ばすURL"
)
...
async def verify_web(
    ...
    view = View()
    view.add_item(Button(label=button_text, url=redirect_url))
    await panel_channel.send(embed=embed, view=view)
    await interaction.response.send_message(
        f"✅ 認証パネルを {panel_channel.mention} に設置しました！", ephemeral=True
    )
